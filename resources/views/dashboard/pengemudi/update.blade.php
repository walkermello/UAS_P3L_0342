    ...
    <div class="container">
        <div class="row">
            <div class="col-md-4 offset-md-4" style="margin-top: 45px;">
                  <h4>Driver Profile</h4><hr>
            
                    <form action="{{route('pengemudi.update')}}" method="post">
                    ...
                    <div class="form-group">
                          ...
                          <input type="text" class="form-control" name="status" placeholder="masukan status" value="{{ $Info->status }}">
                          <span class="text-danger">@error('status'){{ $message }} @enderror</span>
                    </div>
                    <div class="form-group">
                          <button type="submit" class="btn btn-primary">Submit</button>
                          <a href="/pengemudi/delDriver/{{ $Info->id }}" class="btn btn-danger" onclick="return confirm('Anda yakin ingin menghapus akun ini?')">Delete Account</a>
                    </div>
                  </form>
            </div>
        </div>
